    ...
    await Promise.all(
      snapshot.docs.map(async (doc) => {
        ...
        if (doc.data().uberTrip) {
          const selectedUberDispatcher = doc.data().DEBUG
            ? uberDebugDispatcher
            : uberDispatcher;
          await selectedUberDispatcher.auth();
          trip = await selectedUberDispatcher.getTrip(doc.data().uberTrip.id);
        } else if(doc.data().uberFourWheelsTrip) {
          const selectedUberDispatcher = doc.data().DEBUG
            ? uberDebugDispatcher
            : uberFourWheelsDispatcher;
          await selectedUberDispatcher.auth();
          trip = await selectedUberDispatcher.getTrip(doc.data().uberFourWheelsTrip.id);
          trip.isFourWheels = true;
        }
          ...
          if (trip.data.journey.endAt) {
            trip.complete = true;
            trip.status =
              trip.data.journey.endState == 'rider cancel'
                ? 'canceled'
                : 'delivered';
          }
        }
        const journeyDoc = admin
          .firestore()
          .doc('deliveringJourneys/' + doc.id);
        ...
        if (trip.complete) {
          admin
            ...
            .update({
              ...
              reason:
                trip.status == 'returned' ? trip.undeliverable_reason : '',
            });
        } else {
        ...
        if(trip.isFourWheels){
          ...
          .update({
            status: trip.status,
            updatedAt: new Date(),
            uberFourWheelsTrip: trip,
          });
        } else if(doc.data().providerId == 1) {
          await admin
            .firestore()
            .doc(
              'SPREE_ORDERS_' +
