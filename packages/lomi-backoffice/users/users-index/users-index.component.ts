...
import {
  ...
  Query,
  Update,
} from 'packages/lomi-backoffice/ngrx/actions/users.actions';
...
import {
  ...
  UserPrivelege,
  UserRol,
} from 'packages/lomi-backoffice/types/user';
...
export class UsersIndexComponent implements OnInit {
  ...
  constructor(
    ...
    this.appUnsubscribe = this.store.select('app').subscribe((app: App) => {
      this.rols =
        this.userStockLocationId && this.userStockLocationId != -1 && this.userStockLocationId != undefined
          ? app.userRols.filter(
              (rol) => rol.stockLocationId == this.userStockLocationId
            )
          : app.userRols;
    ...
    this.userUnsubscribe = this.store
      ...
      .subscribe((user: IUser) => {
        this.userStockLocationId =
          user.userRol == 1 ? -1 : user.stockLocationId;
        this.changeStockLocation();
      });
    this.usersUnsubscribe = this.store
      .select('users')
      .pipe(map((users: any) => Object.values(users.entities)))
      .subscribe((users: any) => {
        this.users = users;
        const user = this.users.find(
          (user) => user.uid == this.router.url.split('#')[1]
        );
